...
This program has been used to translate the wav file,
generated by the virtual oscilloscope, to dat format.

Operational System: XP, Win7, Win8, Linux.
...
class ConvertWave2Data(QDialog):
    ...
    def changePathToWrite(self, use_same_path=0):
        ...
        if use_same_path == 2:
            path = os.path.join(os.path.dirname(self.path_to_read), os.path.splitext(os.path.basename(self.path_to_read))[0]) + '.dat'
    ...
    def writeData(self, path=''):
        ...
        for index in range(0, self.number_of_frames + 2, 2):
            ...
            file.write(str(float(index) * (1.0 / self.frame_rate)) + '    ' + str(SLInt16("foo").parse(self.frames[index:index + 2])))
